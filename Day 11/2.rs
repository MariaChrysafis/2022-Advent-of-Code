...
pub fn operation (x: &mut i64, s: String) -> i64 {
    let mut inp = s.clone();
    let gamma = (*x).to_string();
    inp = inp.replace("old", &gamma);
    return meval::eval_str(inp).unwrap() as i64;
}
    ...
    for line in stdin.lock().lines() {
        ...
        if &s[2..3] == "S" {
            ...
            for i in new_vec.iter() {
                monkeys[last_index].items.push(i.parse::<i64>().unwrap());
            }
        } else if &s[2..3] == "O" {
            ...
            monkeys[last_index].test = gamma.parse::<i64>().unwrap();
            modulo *= monkeys[last_index].test;
        } else if &s[7..8] == "t" {
